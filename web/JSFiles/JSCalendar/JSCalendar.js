...
/**
  ...
  */
//var _sNeededFilePath = "../../images/";
//var _sNeededFilePath = "JSCalendar/";
//var _sNeededFilePath ="JSFiles/JSCalendar/";

//获取当前路径，用于设定日历路径，确保可正确找到日历图片。
var a=self.location.toString(); //获取当前路径
var b="";
if(a.split("/").length-5>0)
{
	for(j=3;j<a.split("/").length-2;j++)
	{
		b=".."
		b=b+"/";
	}
...
var _sNeededFilePath =b+"JSCalendar/";
...
var _imgBgToday1 = new Image();
	_imgBgToday1.src = _sNeededFilePath + "bgToday2.gif";
...
function openwin(rltvO, year, month, date) 
{
...
function createCldTabFrm(){
	...
	_TD.innerHTML = "<table cellspacing=0 cellpadding=0 class=calendar style='border:0px solid;width:100%'>"
		+ "<tr><td  title=   'Today.'  style='cursor:hand' onclick=\"parent._bHaveSelectNewValue=true;parent.setTargetFormaValue(" + _dftD.getFullYear() + "," + (_dftD.getMonth() + 1) + "," + _dftD.getDate() + ");parent.hideCldTabFrm();return;parent.fillCldTabFrm(" + _dftD.getFullYear() + "," + _dftD.getMonth() + "," + _dftD.getDate() + ");\">"
		+ "<b>&nbsp;<img src='"+_sNeededFilePath+"/true.gif'  width=30px>Today : " + _dftD.getFullYear() + "-" + (_dftD.getMonth() + 1) + "-" + _dftD.getDate()
		+ "</td>"
		+ "<td width=10px>"
		+ "<img style='cursor:hand' onclick=parent.resetTargetValue() title='Clear.' src='"+_sNeededFilePath+"/fault2.gif' onmouseover=this.src='"+_sNeededFilePath+"/fault2.gif' onmouseout=this.src='"+_sNeededFilePath+"/fault2.gif'></td>"
//		+ "<td align=right class=smallFont> Calendar by Jason</td></tr></table>";
...
//				_cldTabFrm.rows(iRow).cells(i).background = "../../images/bgCurDate.gif";
